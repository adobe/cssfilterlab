$(function () { 
    ...
    var timeline = buildTimeline($("#timeline"), $("#timeline .keyframes_list"), $("#timeline .play_head"),
    ...
    function getFiltersForKeyframe(keyframe, theme) {
        ...
        for (var i = 0; i < shaders.length; ++i) {
            ...
            if (!shader.el.hasClass("current"))
                continue;
            var generator = shader.type ? customFilterForBuiltinType : customFilterForShader;
            filterProperty.push(generator(shader, keyframe[shader.name], theme));
        }
    ...
    function updateFilters() {
        ...
        target.css("-webkit-filter-margin", filterMargins)
              .css("-webkit-filter", filterProperty.join(" "));
