    ...
    Filter.prototype = {
        ...
        generateCodeForShader: function(values, colors) {
            var shaderParams = [],
                shaders = [],
                self = this;
            shaders.push(this.config.hasVertex ? colors.fn("url", [colors.uri("shaders/vertex/" + this.name + ".vs")]) : colors.keyword("none"));
            if (this.config.hasFragment || !this.config.hasVertex)
                shaders.push(colors.fn("url", [colors.uri("shaders/fragment/" + this.name + ".fs")]));
