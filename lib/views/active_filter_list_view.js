(function(){
	...
	ActiveFilterListView.prototype = {
        ...
        setupListeners: function(){ 
            ...
            var filterDock = $("#filter-dock"),
                filterDockToggle = $("#filter-store-toggle"),
                self = this;
                              
            filterDock.hide()
            
            $(document).on('click', function(e){
               if (!$(e.target).closest(filterDock).length){
                   filterDock.hide()
               }
            })    
            
            filterDockToggle.on('click', function(e){  
                e.stopPropagation()
                if (filterDock.is(":visible")){
                    filterDock.hide()
                }
                else{
                    filterDock.show()
