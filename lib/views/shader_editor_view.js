(function() {
	...
	ShaderEditorView.prototype = {
        ...
        addUniform: function(uniform) {
            ...
            if (defaultValue && !this.filter.original.params.hasOwnProperty(uniform.name)) {
                this.filter.original.params[uniform.name] = defaultValue;
                this.filter.original.config[uniform.name] = config;
                hadChanges = true;
            }
            if (config && 
                    (!this.filter.original.config.hasOwnProperty(uniform.name)
                    || this.filter.original.config.type != config.type)) {
                this.filter.original.config[uniform.name] = config;
                hadChanges = true;
            }
            return hadChanges;
        ...
        updateFilterConfig: function() {
            this.configEditor.setValue(JSON.stringify(filter.original, null, 4));
            this.filter.reload(filter.original);
        },
