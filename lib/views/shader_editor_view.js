	...
	ShaderEditorView.prototype = {
		...
		show: function() {
			if (!this.panelsAdded) {
                ...
                this.dockView.on("columnResized", this.onEditorResized.bind(this));
                this.onEditorResized.call(this);
	            window.addEventListener("resize", this.onWindowResizedCallback, false);
			}
		},
		
        onEditorResized: function(e, data){
            data = data || {
                "width": $(this.editorColumn.el).css("width")
            }                     
            $("body").trigger("editorResized", [data])
        },
		...
		onColumnClosed: function() {
			...
			this.panelsAdded = false;
			this.onEditorResized.call(this, null, {width: 0});
		},
