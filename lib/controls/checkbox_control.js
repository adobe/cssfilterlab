...
(function() {
    ...
    CheckboxControl.prototype.init = function() {
        ...
        var name = "checkbox-" + this.name + (CheckboxControl.lastEditorId++);
        this.ctrl = $("<input type='checkbox' />")
                    .attr("id", name)
                    .attr("name", name);
        this.ctrlLabel = $("<label />").attr("for", name).text(this.name);
    ...
    CheckboxControl.prototype.pushControls = function(parent) {
        this.pushTableColumns(parent, [this.ctrlParent, this.label]);
    }
